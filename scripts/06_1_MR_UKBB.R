#' ---
#' title: "Cis-MR: PCSK9 --> LDL-C, leave-one-out"
...
#' 
#' I test this five times in all subgroups with valid SNPs using the following SNP sets: 
#' 
#' - the 4 SNPs at the *PCSK9* gene locus 
#' - only 3 of 4 SNPs at the *PCSK9* gene locus (all four combinations)
#' 
...
result.2 = result.2[candidateGene == "PCSK9"]
loaded = load("../temp/06_LDLC_CAD_assoc_stratified_indepSignals.RData")
dat2 = get(loaded)
dat2 = dat2[grepl("LDLC",pheno),]

data_GWAS = result.2[rsID %in% dat2$ID,]
...
myMRTab[FStat.PCSK9<10, c(2,3,8,10,12)]
...
save(myMRTab,file = paste0("../temp/06_MR_input_UKBB_",tag2,".RData"))
load(paste0("../temp/06_MR_input_UKBB_",tag2,".RData"))
...
save(myMRTab,file = paste0("../results/06_MR_WaldEstimates_UKBB_",tag2,".RData"))
...
save(myMRTab_results,file = paste0("../results/06_MR_IVWEstimates_UKBB_",tag2,".RData"))
...
filename1 = paste0("../figures/MRPlots/06_MR_ForerstPlots_acrossPhenos_inclEgger_UKBB_",tag2,".tiff")
...
filename2 = paste0("../figures/MRPlots/06_MR_ForerstPlots_acrossPhenos_UKBB_",tag2,".tiff")
...
for(i in 1:8){
  ...
  filename1 = paste0("../figures/MRPlots/06_MR_ForerstPlots",myPhenos[i],"_UKBB_",tag2,".tiff")
  ...
  par(font=2)
  addpoly(x = c(myTab2_filt[,beta_IVW]),
          sei =  c(myTab2_filt[,SE_IVW]),
          row=c(-0.5,-1.5,-2.5,-3.5,-4.5),
          cex=1,
          mlab=c("Fixed Effect",myTab2_filt[2:5,rsID]))
  dev.off()
...
IATab_egger[setting == "w/o rs11583680",table(IA_pval<0.05,type)]

save(IATab_IVW, file = paste0("../results/06_MR_InteractionTest_UKBB_",tag2,".RData"))
...
tiff(filename = paste0("../figures/MRPlots/06_InteractionScatterPlot_UKBB_",tag2,".tiff"),
