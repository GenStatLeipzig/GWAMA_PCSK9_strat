...
#' @param path_ldsc_data path to LDSC example data to filter SNPs
...
LDSC_PrepInputData_PCSK9 = function(path_data_trait1,
  ...
  erg1[,chrPos := paste(chr,bp_hg19,sep=":")]
  ...
  data.table::setnames(erg1,"SE","se")
  data.table::setnames(erg1,"pval","Pvalue")
  data.table::setnames(erg1,"phenotype","pheno")
  erg1[,Zscore:= beta/se]
  ...
  if(path_data_trait2 != "none"){
    ...
    dum1 = dum1[is.element(markername,dum2$markername),]
    dum2 = dum2[is.element(markername,dum1$markername),]
    matched = match(dum1$markername,dum2$markername)
    dum2 = dum2[matched,]
  ...
  filt1 = erg1_p1$EA == SNPlist$A1 & erg1_p1$OA == SNPlist$A2
  filt2 = erg1_p1$OA == SNPlist$A1 & erg1_p1$EA == SNPlist$A2
  ...
  data.table::setnames(erg1_p1,"EA","A1")
  data.table::setnames(erg1_p1,"OA","A2")
  data.table::setnames(erg1_p1,"bp_hg19","pos")
  
  # Step 7: get necessary columns
  pheno = unique(erg1_p1$pheno)
  myNames = c("chr","pos","snpid","A1","A2","EAF","info","beta","se","Zscore","N","Pvalue")
  ...
  if(path_data_trait2 != "none"){
    ...
    filt1 = erg1_p2$EA == SNPlist$A1 & erg1_p2$OA == SNPlist$A2
    filt2 = erg1_p2$OA == SNPlist$A1 & erg1_p2$EA == SNPlist$A2
    message("     Checking matching alleles: \n        ",sum(filt1)," SNPs are a direct match (same coding) \n        ",sum(filt2)," SNPs have to be transformed (reversed coding)")
    erg1_p2 = erg1_p2[filt1 | filt2, ]
    message("     After filtering for matching alleles in LDSC SNPlist, ",dim(erg1_p2)[1]," SNPs remain ...")
    
    data.table::setnames(erg1_p2,"EA","A1")
    data.table::setnames(erg1_p2,"OA","A2")
    data.table::setnames(erg1_p2,"bp_hg19","pos")
