ThreeWayInteractionTest_jp = function(data,pheno1,pheno2,pheno3,pheno4,data_type="GWAMA",suffix ="IVW"){
  ...
  if(data_type == "GWAMA"){
    ...
    # build information
    result.3 = copy(data1)[,c(3)]
    
    result.3[,trait1 := pheno1]
    result.3[,trait1_beta := data1[,get(paste0("beta_",suffix))] ]
    result.3[,trait1_SE := data1[,get(paste0("SE_",suffix))] ]
    result.3[,trait1_pval := data1[,get(paste0("pval_",suffix))] ]

    result.3[,trait2 := pheno2]
    result.3[,trait2_beta := data2[,get(paste0("beta_",suffix))] ]
    result.3[,trait2_SE := data2[,get(paste0("SE_",suffix))] ]
    result.3[,trait2_pval := data2[,get(paste0("pval_",suffix))] ]

    result.3[,trait3 := pheno3]
    result.3[,trait3_beta := data3[,get(paste0("beta_",suffix))]]
    result.3[,trait3_SE := data3[,get(paste0("SE_",suffix))] ]
    result.3[,trait3_pval := data3[,get(paste0("pval_",suffix))] ]
    
    result.3[,trait4 := pheno4]
    result.3[,trait4_beta := data4[,get(paste0("beta_",suffix))] ]
    result.3[,trait4_SE := data4[,get(paste0("SE_",suffix))] ]
    result.3[,trait4_pval := data4[,get(paste0("pval_",suffix))] ]
