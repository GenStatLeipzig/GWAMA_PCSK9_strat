manhattanPlot<-function(x,ymax=NULL,ymin=NULL, 
                    title="Manhattan Plot",xlabel="chromosome",
                    ylabel=expression(paste("TWAS: ",log[10](p)," and GWAS: ",-log[10](p))),
                    hline1=NULL,sugline1=NULL,highlight=F, 
                    returnObject = F,suppress19_21label =F, diffsize = FALSE , num_breaks_y=5, 
                    mypointsize=1,
                    showcolors =F, overall_max = 20,plotGenes=T) {
  ...
  if(plotGenes==T){
    ...
    myPD<-data.table(myX,myY,myColor, myLabel,myFlag,myPhenotype,myGene)
  }else{
  ...
  if(highlight==T) {
    ...
    myPD[abs(myY)>6 & myPhenotype == "females", myColor:=3]
    myPD[abs(myY)>6 & myPhenotype == "free", myColor:=4]
    myPD[abs(myY)>6 & myPhenotype == "males", myColor:=5]
    myPD[abs(myY)>6 & myPhenotype == "treated", myColor:=6]
    myPD[abs(myY)>6 & myPhenotype == "females free", myColor:=7]
    myPD[abs(myY)>6 & myPhenotype == "males free", myColor:=8]
    myPD[abs(myY)>6 & myPhenotype == "females treated", myColor:=9]
    myPD[abs(myY)>6 & myPhenotype == "males treated", myColor:=10]
  }
  ...
  if(diffsize == T & is.null(sugline1)==F){ myPD[myY>sugline1 ,pointsize:=2]}
  ...
  myPlot <- myPlot + geom_point(data=myPD, 
                                aes(x=myX, y=myY, colour=myColor, size = pointsize,label = myGene)) + 
  ...
  if(is.null(highlight) & showcolors!= F) {
    myPlot <- myPlot + scale_colour_manual(values= rep(rev(brewer.pal(5, "Dark2")), 23))
      ...
      myPlot <- myPlot + scale_colour_manual(values=c("black","#FDAE61","darkgrey","#F46D43","#005A32","#74ADD1","#41AB5D","#D73027","#4575B4","#ABD9E9")) 
      }
  ...
  # add genes
  myPlot2 <-myPlot + 
    geom_label_repel(data = subset(myPD, myY>=6 & !is.na(myGene)),
                     aes(x=myX, y=myY, label = myGene),ylim = c(12,ymax),nudge_y = +1)
