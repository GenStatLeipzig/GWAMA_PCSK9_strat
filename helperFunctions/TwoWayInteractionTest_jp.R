TwoWayInteractionTest_jp = function(data,pheno1,pheno2,type,useBestPheno,corCol,data_type="GWAMA",suffix="IVW"){
  # data = copy(result.2)
  # pheno1 = "females"
  # pheno2 = "males"
  # type = "sexIA"
  ...
  if(useBestPheno == T){
    ...
    data1 = data1[phenotype == paste0("PCSK9_",pheno1)]
    data2 = data2[phenotype == paste0("PCSK9_",pheno2)]
  }
  ...
  if(data_type == "GWAMA"){
    ...
    result.3[,IA_diff := trait2_beta - trait1_beta]
    result.3[,IA_SE := sqrt(trait1_SE^2 + trait2_SE^2) ]
    result.3[,IA_Zscore := IA_diff/IA_SE ]
    result.3[,IA_pval := pnorm(abs(IA_Zscore), lower.tail = F)*2 ]
  }
